...
const normalizePaymentMethod = (method) => {
  ...
  if (method.toLowerCase() === 'cssh') return 'Cash';
  return method.split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1).toLowerCase()).join(' ');
};

// Denormalize for database
const denormalizePaymentMethod = (method) => {
  if (method.toLowerCase() === 'cash') return 'Cssh';
  if (method.toLowerCase() === 'bank transfer') return 'Bank Transfer';
  return method;
};
...
const Reconciliation = () => {
  ...
  const [showChecks, setShowChecks] = useState(false); // 👈 Toggle state
  ...
  // Fetch payment methods directly from dynamic_sales
  const fetchPaymentMethods = useCallback(async () => {
    ...
    if (error) {
      ...
      const uniqueMethods = [...new Set(data.map(item => item.payment_method))].sort();
      console.log('Fetched payment methods:', uniqueMethods);
  ...
  const fetchSales = useCallback(async () => {
    ...
    const startDate = timePeriod === 'daily' ? startOfDay(new Date(checkDate)) :
                     timePeriod === 'weekly' ? startOfWeek(new Date(checkDate), { weekStartsOn: 1 }) :
                     startOfMonth(new Date(checkDate));
    const endDate = timePeriod === 'daily' ? new Date(checkDate) :
                    timePeriod === 'weekly' ? subDays(subWeeks(startDate, -1), 1) :
                    subDays(subMonths(startDate, -1), 1);
  ...
  const fetchReconciliationChecks = useCallback(async () => {
    ...
    if (error) {
      ...
      console.log('Fetched checks:', data);
      setReconciliationChecks(data || []);
    }
    setIsLoading(false);
  }, [storeId]);

  // Aggregate sales by payment method for summary and modal
  ...
  const totalSalesAmount = Object.values(salesByPaymentMethod).reduce((sum, { amount }) => sum + amount, 0);
  if (salesByPaymentMethod.Cssh && salesByPaymentMethod.Cssh.amount > totalSalesAmount * 0.5) {
    suspiciousPatterns.push('High cash transactions detected (>50% of total sales). Verify cash deposits.');
  }
  const nonSoldSales = sales.filter(s => s.status !== 'sold').length;
  if (nonSoldSales > sales.length * 0.1) {
    suspiciousPatterns.push('Frequent non-sold transactions detected. Check for voids or cancellations.');
  }

  // Chart data for expected vs actual
  const chartData = {
    labels: Object.keys(salesByPaymentMethod).map(normalizePaymentMethod),
    datasets: [
      {
        label: `Expected Money (${currencySymbols[currency]})`,
        data: Object.keys(salesByPaymentMethod).map(method => salesByPaymentMethod[method].amount),
        backgroundColor: '#10B981',
      },
      {
        label: `Actual Money (${currencySymbols[currency]})`,
        data: Object.keys(salesByPaymentMethod).map(method => {
          const check = reconciliationChecks.find(c => c.payment_method === method.toLowerCase() && c.check_date === checkDate);
  ...
  const handleAddCheck = async () => {
    ...
    const { error } = await supabase
      ...
      .insert({
        ...
        expected_amount: newCheck.expected_amount,
  ...
  // Fetch sales, checks, and payment methods when filters change
  useEffect(() => {
    if (storeId) {
      fetchPaymentMethods();
      fetchSales();
  ...
  // Pre-populate expected amount in modal when payment method changes
  useEffect(() => {
    if (newCheck.payment_method && salesByPaymentMethod[denormalizePaymentMethod(newCheck.payment_method)]) {
      setNewCheck(prev => ({
        ...prev,
        expected_amount: salesByPaymentMethod[denormalizePaymentMethod(newCheck.payment_method)].amount,
      }));
    }
  }, [newCheck.payment_method, salesByPaymentMethod]);

  return (
    <div className="p-2 bg-gradient-to-br from-gray-50 to-gray-100 min-h-screen transition-all duration-300">
      ...
      <div className="mb-8 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-4 bg-white p-6 rounded-xl shadow-lg">
        ...
        <div>
          ...
          >
            ...
            {paymentMethods.map(method => (
              <option key={method} value={method}>{normalizePaymentMethod(method)}</option>
            ))}
          </select>
        </div>
        <div>
        ...
        <button
          ...
          onClick={() => {
            ...
            setNewCheck({
              payment_method: selectedPaymentMethod ? normalizePaymentMethod(selectedPaymentMethod) : '',
              expected_amount: selectedPaymentMethod && salesByPaymentMethod[selectedPaymentMethod] ? salesByPaymentMethod[selectedPaymentMethod].amount : 0,
              actual_amount: 0,
              notes: '',
              status: 'pending',
            });
        ...
        <button
          ...
          onClick={() => downloadCSV(reconciliationChecks.map(c => ({
            ...c,
      ...
      {/* Sales Table */}
      {storeId && (
        <div className="mb-8 bg-white p-2 rounded-xl shadow-md">
          <h2 className="text-lg font-semibold text-gray-700 mb-4">All Sales</h2>
          {sales.length === 0 ? (
      ...
      {suspiciousPatterns.length > 0 && (
        <div className="mb-8 bg-yellow-100 p-6 rounded-xl shadow-md">
          <h2 className="text-lg font-semibold text-yellow-700 mb-2">⚠️ Things to Check</h2>
...
{storeId && (
    <div className="mb-8 bg-white p-2 rounded-xl shadow-md">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-lg font-semibold text-gray-700">Money Received by Payment Method</h2>
          <button
            ...
            <>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mb-4">
                {Object.entries(salesByPaymentMethod).map(([method, data]) => (
                  <div
                    key={method}
                    className="bg-gray-50 p-3 rounded-md text-center shadow-sm"
                  >
      ...
      {storeId && (
        ...
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-lg font-semibold text-gray-700">Saved Checks</h2>
          <button
                ...
                <thead>
                  <tr className="bg-gray-100">
                    ...
                    <th className="p-3 text-left text-gray-700 font-semibold">
                      Expected ({currencySymbols[currency]})
                    </th>
                    <th className="p-3 text-left text-gray-700 font-semibold">
                      Actual ({currencySymbols[currency]})
                    </th>
                    <th className="p-3 text-left text-gray-700 font-semibold">
                      Discrepancy ({currencySymbols[currency]})
                    </th>
      ...
      {showAddModal && (
        <div className="fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-50 z-50">
          <div className="bg-white p-2 rounded-xl shadow-lg max-w-md w-full">
            <h2 className="text-lg font-semibold text-gray-700 mb-4">Add New Check</h2>
            <div className="space-y-4">
              <div>
                ...
                <select
                  ...
                  disabled={!storeId || sales.length === 0}
      ...
      {showEditModal && selectedCheck && (
        <div className="fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-50 z-50">
          <div className="bg-white p-2 rounded-xl shadow-lg max-w-md w-full">
            <h2 className="text-lg font-semibold text-gray-700 mb-4">Edit Check</h2>
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700">Payment Method</label>
                <input
      ...
      {showDeleteModal && selectedCheck && (
        <div className="fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-50 z-50">
          <div className="bg-white p-6 rounded-xl shadow-lg max-w-sm w-full">
            <h2 className="text-lg font-semibold text-gray-700 mb-4">Delete Check?</h2>
